...
// Navigation phrasing is now generated by Gemini; Nemotron remains the intent parser.
    ...
    : INavigationSpeechService
{
    public async Task<string> PhraseAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (!GeminiTextGeneration.IsConfigured(configuration))
            throw new InvalidOperationException("The configured Gemini API key is unavailable.");

        return await GeminiTextGeneration.GenerateStructuredTextAsync(
            configuration,
            """
            ...
            - Put only the final spoken sentence in the template field.

            Tone examples only (never copy facts from them):
