...
interface AiApi { @POST("api/AI/ask") suspend fun ask(@Body request: AssistantRequest): Response<AssistantResponseDto> }
interface AiRepository { suspend fun ask(request: AssistantRequest): ApiResult<AssistantResponseDto> }
class AiRepositoryImpl(private val api: AiApi, private val sessions: AuthSessionStore, private val errors: ApiErrorParser) : AiRepository {
    override suspend fun ask(request: AssistantRequest) = authenticatedApiCall(sessions, errors) { api.ask(request) }
}
