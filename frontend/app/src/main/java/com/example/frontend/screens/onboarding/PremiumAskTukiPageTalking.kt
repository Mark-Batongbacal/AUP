...
private enum class TalkingBudgetMode {
    WALK,
    TRICYCLE,
    JEEPNEY,
    DESTINATION
...
/**
 * Page 4 version that uses tuki_onboarding_talking1 / tuki_onboarding_talking2 as mouth frames.
 * The two frames are resolved by resource name so this branch still compiles before the PNG files
 * are pushed. Once those exact drawable names exist, the talking animation activates automatically.
 * tuki_onboarding_page4 is intentionally not used here; it remains reserved for the handoff screen.
 */
@Composable
fun PremiumAskTukiPageTalking(active: Boolean) {
    ...
    TalkingBudgetPageShell {
        TalkingBudgetPageTitle(
        ...
        AnimatedVisibility(
            ...
            Row(
                ...
                Surface(
                    ...
                    color = TukiTeal.copy(alpha = 0.08f),
                    shape = CircleShape,
...
private fun TalkingTukiImage(
    ...
    val talking1 = remember(resources, packageName) {
        resources.getIdentifier("tuki_onboarding_talking1", "drawable", packageName)
    }
    val talking2 = remember(resources, packageName) {
        resources.getIdentifier("tuki_onboarding_talking2", "drawable", packageName)
    }
    val talkingFramesAvailable = talking1 != 0 && talking2 != 0

    // Do not fall back to tuki_onboarding_page4 because that pose is reserved for the next screen.
    val fallback = R.drawable.tuki_pose_celebrate
    val firstFrame = if (talkingFramesAvailable) talking1 else fallback
    val secondFrame = if (talkingFramesAvailable) talking2 else fallback

    var showFirstFrame by remember { mutableStateOf(false) }

    LaunchedEffect(talking, firstFrame, secondFrame) {
        if (!talking || firstFrame == secondFrame) {
    ...
    val frame = if (talking && showFirstFrame) firstFrame else secondFrame
...
@Composable
private fun TalkingBudgetReplyingDots() {
    val transition = rememberInfiniteTransition(label = "talking_budget_replying_dots")
    val firstDot by transition.animateFloat(
        initialValue = 0.30f,
        targetValue = 1f,
        animationSpec = infiniteRepeatable(
            animation = tween(durationMillis = 340, delayMillis = 0),
            repeatMode = RepeatMode.Reverse
        ),
        label = "talking_budget_dot_1"
    )
    val secondDot by transition.animateFloat(
        initialValue = 0.30f,
        targetValue = 1f,
        animationSpec = infiniteRepeatable(
            animation = tween(durationMillis = 340, delayMillis = 110),
            repeatMode = RepeatMode.Reverse
        ),
        label = "talking_budget_dot_2"
    )
    val thirdDot by transition.animateFloat(
        initialValue = 0.30f,
        targetValue = 1f,
        animationSpec = infiniteRepeatable(
            animation = tween(durationMillis = 340, delayMillis = 220),
            repeatMode = RepeatMode.Reverse
        ),
        label = "talking_budget_dot_3"
    )
...
@Composable
private fun TalkingBudgetRouteResultCard() {
    Surface(
        modifier = Modifier.fillMaxWidth(),
        color = Color.White,
        ...
        Column(modifier = Modifier.padding(horizontal = 17.dp, vertical = 15.dp)) {
            ...
            Row(
                ...
                TalkingBudgetRouteStep(TalkingBudgetMode.WALK, "Walk", "6 min")
                TalkingBudgetArrow()
                TalkingBudgetRouteStep(TalkingBudgetMode.TRICYCLE, "Tricycle", "8 min")
                TalkingBudgetArrow()
                TalkingBudgetRouteStep(TalkingBudgetMode.JEEPNEY, "Jeepney", "13 min")
                TalkingBudgetArrow()
                TalkingBudgetRouteStep(TalkingBudgetMode.DESTINATION, "SM Clark", "")
            }
        }
    }
}
...
@Composable
private fun TalkingBudgetRouteStep(
    mode: TalkingBudgetMode,
    label: String,
    detail: String
) {
    Column(
        ...
        TalkingBudgetRouteIcon(mode = mode, modifier = Modifier.size(29.dp))
...
@Composable
private fun TalkingBudgetArrow() {
    Text(
        text = "→",
...
@Composable
private fun TalkingTukiResultRow() {
    Row(
        modifier = Modifier.fillMaxWidth(),
        verticalAlignment = Alignment.CenterVertically,
...
@Composable
private fun TalkingCelebrationMarks(modifier: Modifier = Modifier) {
    Canvas(modifier = modifier) {
        ...
        marks.forEachIndexed { index, center ->
            ...
            drawLine(color, Offset(center.x - radius, center.y), Offset(center.x + radius, center.y), 2.dp.toPx(), StrokeCap.Round)
            drawLine(color, Offset(center.x, center.y - radius), Offset(center.x, center.y + radius), 2.dp.toPx(), StrokeCap.Round)
        }
    }
}

@Composable
private fun TalkingBudgetPageShell(content: @Composable () -> Unit) {
...
@Composable
private fun TalkingBudgetDecor() {
...
@Composable
private fun TalkingBudgetPageTitle(title: String, subtitle: String) {
    Text(
        text = title,
        color = TukiInk,
...
@Composable
private fun TalkingBudgetRouteIcon(
    mode: TalkingBudgetMode,
    modifier: Modifier = Modifier
) {
    Canvas(modifier = modifier) {
